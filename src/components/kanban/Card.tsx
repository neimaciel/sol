...
export function CardContent({ card, isOverlay, onClick, onExpand, isExpanded }: {
    ...
    return (
        <motion.div
            ...
            className={cn(
                "group relative transition-all duration-200 border-2 overflow-hidden bg-card",
                isOverlay
        ...
        >
            ...
            <div className={cn("pl-5 space-y-2", isCompactMode ? "p-2" : "p-3")}>
                {/* Compact Header */}
                <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                        <span className="text-[10px] font-mono font-bold text-foreground bg-muted px-1 py-0.5 border border-border">
                            #{card.id.substring(0, 4)}
                        </span>
                        <h4 className={cn("font-heading font-bold text-foreground truncate uppercase", isCompactMode ? "text-[10px]" : "text-xs")}>
                    ...
                    <Button
                        variant="ghost"
                        size="icon"
                        className="h-6 w-6 text-muted-foreground hover:text-foreground hover:bg-accent -mr-1 transition-colors rounded-none"
                        onClick={onExpand}
                    >
                ...
                <AnimatePresence>
                    {(isExpanded || !isCompactMode) && (
                        ...
                        >
                            <div className="space-y-3 pt-2 border-t-2 border-border/50 border-dashed mt-2">
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                            <Calendar className="w-3.5 h-3.5 text-foreground" strokeWidth={2} />
                                            <span className="font-bold font-mono text-foreground">{card.date}</span>
                                        </div>
                                        <div className={cn(
                                            "px-2 py-0.5 text-[10px] font-black uppercase tracking-wide border-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]",
                                            card.priority === 'high'
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                            <DollarSign className="w-3.5 h-3.5 text-emerald-600" strokeWidth={2.5} />
                                ...
                                <div className="flex items-center justify-between pt-1">
                                    <div className="flex items-center gap-1">
                                        <div className="w-5 h-5 bg-primary text-primary-foreground border-2 border-foreground flex items-center justify-center text-[8px] font-bold">
                                            RS
                                        </div>
                                    </div>
                                    <span className="text-[10px] text-foreground font-bold uppercase hover:underline decoration-2 underline-offset-2">Ver detalhes</span>
                                </div>
                            </div>
                        </motion.div>
                    )}
